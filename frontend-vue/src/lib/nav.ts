/**
 * 侧栏导航模型:组 -> 条目(判别联合)。
 * 加组/加条目 = 数组加一行;未来新增条目 kind 只需 SideNav.vue 的 v-for 增一个分支。
 * 新增"视图"另需扩 lib/route.ts 的 RouteView 联合 + router/index.ts 的路由表。
 */
...
export interface NavLinkEntry {
  ...
  /** hash 目标(如 "#/")——不新建路由 API */
  to: string;
  /** view 宽化:新视图只需在本文件加行 + route.ts 加分支 */
  isActive?: (view: RouteView) => boolean;
}
