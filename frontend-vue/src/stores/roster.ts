 ...
 */
import { ref } from "vue";
import { fetchRoster } from "../api/obs";
import { diffLabel } from "../lib/format";
import type { ChallengeRow, LocalTrace, RosterSnapshot } from "../types";

export const rosterData = ref<RosterSnapshot | null>(null);
export const rosterFailed = ref(false);
  ...
  if (!inflight) {
    inflight = (async () => {
      try {
        ...
        if (stopped) return;
        rosterData.value = snap;
        rosterFailed.value = false;
      } catch {
        // 有旧数据时保持展示,语义同旧 Overview
        if (!stopped) rosterFailed.value = true;
      } finally {
        inflight = null;
      }
    })();
...
export function toRows(snap: RosterSnapshot | null | undefined): ChallengeRow[] {
