...
本地目录扫描/FLAG 读法/单行模板在 worker.dashboard(纯 stdlib);
此处从那里导入,保证两处读到的目录语义一致。
本模块与 dashboard 同属 worker 模块内部,不存在跨模块 import。
"""
...
from redpilot.contracts.fsio import atomic_write_json
from redpilot.contracts.paths import LIVE_DIR, ROSTER_FILENAME

from redpilot.worker.dashboard import (challenge_detail, empty_roster_snapshot,
                             local_challenge_row, read_flag_lines, scan_local)

log = logging.getLogger("redpilot.worker.roster")
