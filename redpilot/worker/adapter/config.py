...
# ── 求解身份隔离（M1/M2，设计 docs/solver-isolation-design.md §4）──

@dataclass
class IsolationConfig:
    """求解进程降权配置。

    enabled=False 即回退到历史行为（driver 与 Pi 同为 root）；strict=True 时
    前置条件缺失会 fail closed（调用方走 exit 0 语义，见 I8）。
    """
    enabled: bool = True
    strict: bool = False
    user: str = "solver"
    ...
    @classmethod
    def from_env(cls) -> "IsolationConfig":
        return cls(
            enabled=(_env("ADAPTER_ISOLATION", "1") != "0"),
            strict=(_env("ADAPTER_ISOLATION_STRICT", "0") == "1"),
            user=(_env("ADAPTER_SOLVER_USER", "solver") or "solver"),
        )
...
@dataclass
class SurfaceBudgetConfig:
    """SurfaceLedger 的阈值配置（全部秒/次数，确定性）。"""
    enabled: bool = True
    soft_seconds: int = 180      # L0：停滞软信号
    hard_seconds: int = 300      # L1：关闭该面（对齐 AGENTS.md 的 5 分钟）
    abs_seconds: int = 900       # 单面绝对上限（有事实也算）
    ignore_seconds: int = 120    # L2 观察窗：L1 后仍在该面
    ignore_calls: int = 3        # L2：观察窗内该面调用数
    max_surfaces: int = 64       # 每 code 保留的 surface 数（LRU）

    @classmethod
    def from_env(cls) -> "SurfaceBudgetConfig":
        def _int(name: str, default: int) -> int:
            try:
                return max(0, int(_env(name, str(default)) or default))
            except (TypeError, ValueError):
                return default

        return cls(
            enabled=(_env("ADAPTER_SURFACE_BUDGET", "1") != "0"),
            soft_seconds=_int("ADAPTER_SURFACE_STALL_SOFT", 180),
            hard_seconds=_int("ADAPTER_SURFACE_STALL_HARD", 300),
            abs_seconds=_int("ADAPTER_SURFACE_ABS_S", 900),
            ignore_seconds=_int("ADAPTER_SURFACE_IGNORE_S", 120),
            ignore_calls=_int("ADAPTER_SURFACE_IGNORE_CALLS", 3),
            max_surfaces=_int("ADAPTER_SURFACE_MAX", 64) or 64,
        )
...
@dataclass
class WorkGcConfig:
    """题目目录回收配置：默认只报告（dry-run），确认后再开真删。"""
    enabled: bool = True
    apply: bool = False
    retention_days: int = 14

    @classmethod
    def from_env(cls) -> "WorkGcConfig":
        try:
            days = max(1, int(_env("ADAPTER_WORK_RETENTION_DAYS", "14") or "14"))
        except (TypeError, ValueError):
            days = 14
        return cls(
            enabled=(_env("ADAPTER_WORK_GC_ENABLED", "1") != "0"),
            apply=(_env("ADAPTER_WORK_GC", "0") == "1"),
            retention_days=days,
        )
