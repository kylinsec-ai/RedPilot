...
from redpilot.contracts.paths import HARNESS_DIR, HARNESS_STOPLOSS_SUBDIR
...
class StopLoss:
    ...
    def _state_path(self, code: str) -> str:
        """控制面预算状态：只写 `.harness`（M2，设计 §5）。

        此前状态放在题目目录里（`<workdir>/<code>/.stoploss.json`）—— 那正是
        Pi 的 cwd：一次 `rm -rf .` / `git clean -fdx` 就能删掉预算窗口，
        同 uid 也能随手改写。裁决状态与工作记忆必须分开。
        """
        return os.path.join(self.workdir, HARNESS_DIR, HARNESS_STOPLOSS_SUBDIR,
                            f"{self._safe_code(code)}.json")

    def _legacy_state_path(self, code: str) -> str:
        """旧位置（题目目录）。只读兼容一个发布周期，见设计 §5.2。"""
    ...
    def _load(self, code: str):
        ...
        读取顺序：`.harness`（新） → 题目目录（旧，只读兼容）。命旧路径时
        不在这里搬家；下一次 `_save` 自然写到新路径（单向迁移）。
        """
        new_path = self._state_path(code)
        legacy_path = self._legacy_state_path(code)
        d = None
        for candidate in (new_path, legacy_path):
            try:
                with open(candidate, encoding="utf-8") as f:
                    d = json.load(f)
                if candidate == legacy_path:
                    log.info("stoploss state for %s read from legacy path; "
                             "next save migrates it into %s", code, HARNESS_DIR)
                break
    ...
    def reset(self, code: str) -> bool:
        """丢弃某题的持久预算状态（task epoch 变更时调用）。

        M2 把状态搬进 `.harness` 后，旧机制靠“清空题目目录顺带删掉
        `.stoploss.json`”的副作用失效了 —— 新任务沿用旧任务的终态会让
        复用的公开 code 直接 `dropped`。这里显式清，新/旧两处都删。
        """
        removed = False
        for path in (self._state_path(code), self._legacy_state_path(code)):
            try:
                os.unlink(path)
                removed = True
            except FileNotFoundError:
                ...
                log.warning("stoploss reset failed for %s: %s", path, exc)
        with self._local_lock(code):
