...
sys.path，且不在 worker 镜像内），现搬进 `redpilot.worker` 成为包内模块：

- 删掉模块顶部的 `sys.path.insert(...)` —— 包内导入不需要它；
- `redpilot.worker.adapter.*` 的绝对导入保持原样，包内成立；
...
from redpilot.worker.adapter.config import (SolverConfig, ControllerConfig,
...
_STATUS: dict = {
    ...
    # phase 是**给观测面的**（redpilot.contracts.vocabulary.PHASES 的词表）。
...
# ── 框架观测桥（可选）────────────────────────────────────
# 装配层（redpilot.worker.driver）注入一个 StatusBridge，把上面这份状态转译成
...
def _monitor_loop(*, raw_client=None, watch_dir: str = "", stop_event=None):
    ...
    # 完整设计说明见 redpilot.worker/supervisor.py 顶部注释；改之前先读它。
...
def _flag_plaintext_rx(workdir: str):
    ...
    **只读展示**路径（`.heimdall.json` 的 why/evidence 是工具输出摘录，可能含
    ...
    """
    from redpilot.worker.adapter.compliance import flag_plaintext_rx
...
def _purge_stale_solutions(workdir: str, code: str, targets: list,
    ...
    # `.stoploss.json` 是旧布局的遗留位置（M2 后预算状态住在 .harness）：
    # 保留它只为旧 workdir 的兼容读取，新目录不会再产生该文件。
...
def _heimdall_init(llm) -> None:
    ...
    if llm is None:
        ...
        return
    try:
        from redpilot.worker.adapter.heimdall import Heimdall
        _HEIMDALL = Heimdall(
...
def _heimdall_map_for(workdir: str) -> str:
    ...
    if _HEIMDALL is None:
        return ""
    try:
        from redpilot.worker.adapter.heimdall import load_state, render
...
def _heimdall_observe(workdir: str, tpath: str, session_idx: int) -> None:
    ...
    if _HEIMDALL is None:
        return
    try:
        from redpilot.worker.adapter.heimdall import review
...
def _solve_one_unlocked(
    ...
    *,
    ...
    try:
        # epoch 变更 = 旧预算状态必须显式作废：M2 之后预算不在题目目录里，
        # 清目录不再顺带删掉它。
        if _preflight_task_epoch_isolation(
                _preflight_workdir, code, task_epoch=_current_task_epoch()):
            if stoploss.reset(code):
                log.info("  %s preflight reset stale stoploss state", code)
    except Exception:
    ...
    os.makedirs(workdir, exist_ok=True)
    # 把题目目录交给求解身份（ACL 优先、chown 备选）；失败不阻断解题，
    # 降级信息由 isolation.probe/日志表达（设计 §4.5）。
    _iso_mode = isolation.prepare_challenge_dir(
        ctrl.workdir, code, IsolationConfig.from_env(), _safe_code)
    if _iso_mode in ("none", "chown"):
        log.warning("  %s isolation: challenge dir prepared by %s (acl unavailable?)",
                    code, _iso_mode)
    ...
    try:
        while (time.monotonic() < visit_deadline and not stop_event.is_set()
            ...
            # M3：会话内攻击面预算（每场一份内存账本，落盘在 .harness，同 epoch 续用）。
            _surface_cfg = SurfaceBudgetConfig.from_env()
            _surface_ledger = surface.SurfaceLedger(
                _surface_cfg, code, epoch=_current_task_epoch(),
                persist_path=surface.ledger_path(workdir, code, _safe_code))
            ...
            prompt = build_task_prompt(
                ...
                # M1：降权后的能力边界（隔离关闭时为空串）。
                identity_note=isolation.capability_note(IsolationConfig.from_env()),
                # M3：上一场已关闭的攻击面（同 epoch；空串整段不注入）。
                surface_note=_surface_ledger.prompt_note(),
            )
            ...
            def _on_fact(tool, args, output, _nf=new_facts):
                ...
                # M3：面预算的实时输入（影子黑板机械抽取；不碰真实黑板，也不
                # 影响场末那套带证据门的计数）。
                if _surface_cfg.enabled:
                    try:
                        _sa = _surface_ledger.note_result(tool, args, output)
                        if _sa.get("rekey"):
                            obs.emit("surface.rekey", layer="adapter", payload={
                                "code": code, "old": _sa["rekey"],
                                "new": _sa["key"], "target": _sa["target"],
                                "tactic": _sa["tactic"]})
                    except Exception as exc:
                        log.warning("surface ledger note failed: %s", exc)
            ...
            def _surface_control():
                """M3：把账本动作翻成 RPC 帧（由 pi_agent 每个循环周期拉取）。

                关闭与转向是两件事：`surface.closed` 记事实，`surface.rotate`
                记动作；abort 后接 follow_up，让同一个常驻会话带着指令继续，
                而不是把整场上下文丢掉。
                """
                try:
                    act = _surface_ledger.tick()
                except Exception as exc:
                    log.warning("surface tick failed: %s", exc)
                    return []
                if act is None:
                    return []
                payload = {"code": code, "session": session_idx,
                           "surface": act.key, "target": act.target,
                           "tactic": act.tactic, "action": act.kind,
                           "reason": act.reason}
                if act.kind == "soft":
                    obs.emit("surface.soft", layer="adapter", payload=payload)
                    return [{"type": "steer", "message": act.text}]
                if act.kind == "steer" and act.reason in ("stall", "absolute"):
                    obs.emit("surface.closed", layer="adapter", payload=payload)
                obs.emit("surface.rotate", layer="adapter", payload=payload)
                if act.kind == "abort":
                    return [{"type": "abort"},
                            {"type": "follow_up", "message": act.text}]
                return [{"type": "steer", "message": act.text}]
            ...
            # M3：场末落盘 + 汇总（有面才发事件，避免噪音）。
            _surface_ledger.flush()
            _surface_summary = _surface_ledger.session_summary()
            if _surface_summary.get("surfaces"):
                obs.emit("surface.summary", layer="adapter",
                         payload={"code": code, "session": session_idx,
                                  **_surface_summary})
            ...
            if solved:
                ...
                try:
                    ...
                    # M4：打上终态标记（回收年龄基准；白名单保留）。
                    workgc.mark_closed(workdir, "solved")
                except Exception:
...
def main():
    ...
    def _heartbeat_loop():
        ...
        while True:
            ...
            try:
                ...
                pass
            # M4：每 ~2 小时跑一次 /work 保留策略（默认只报告）。心跳线程已经
            # 是唯一的周期性支点，不再另起线程。
            if _gc_cfg.enabled and _ticks % 240 == 0:
                try:
                    obs.emit("work.gc_report", layer="driver",
                             payload=workgc.run_once(ctrl.workdir, _gc_cfg))
                except Exception:
                    log.warning("work gc failed", exc_info=True)

    _gc_cfg = WorkGcConfig.from_env()
    ...
    # 启动隔离自检报告（driver 阶段观测通道未就绪，由这里补报；设计 §4.6）。
    _iso_report = isolation.last_report()
    if _iso_report:
        obs.emit("isolation.probe", layer="driver", payload=_iso_report)
    ...
    if verifier_cfg.is_usable():
        try:
            from redpilot.worker.adapter.llm import LLMClient
