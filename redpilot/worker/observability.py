...
编排主循环（`redpilot.worker.orchestrator`，朋友那份 6,975 行竞技场）有自己的
...
框架的观测面吃的是 `LiveState` 的 18 键快照（`redpilot.contracts.snapshot`），
经 `redpilot.worker.relay` 推到 obs 平台、经 `redpilot.worker.dashboard` 喂 :8080 态势台。
...
（取值就是 `redpilot.contracts.vocabulary.PHASES` 的词表）。本模块只做**改名**，
不做**推导** —— 这一条是刻意的：
...
from redpilot.contracts.text import ERROR_HEAD_MAX, OUTPUT_TAIL_MAX, tail_text
from redpilot.contracts.vocabulary import OUT_OF_BAND_PREFIX, PHASES

log = logging.getLogger("redpilot.worker.observability")
...
class StatusBridge:
    ...
    参数就是框架观测面既有的三个对象：
      - `live`: `redpilot.worker.live.LiveState`（None = 只发总线不发快照）
      - `bus`:  `redpilot.worker.live.LiveBus`  （None = 不广播）
      - `relay`: `redpilot.worker.relay.ObsRelay`（可选；用于把平台确认的明文 flag
        带外送进 Runs 历史）
    """
