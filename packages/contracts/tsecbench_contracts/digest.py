...
此前 workers/roster.py TranscriptDigest._feed(增量文件解析用)与 obs/digest.py
(DB 事件行全量重折叠用)是两台必须手工同步的状态机;现归一:本模块持唯一语义,
两侧各留 I/O 壳(worker: 增量 offset 续读 + sidecar 缓存;obs: 无状态 fold_rows)。
...
log = logging.getLogger("tsecbench_contracts.digest")
...
class FoldState:
    ...
    worker 侧 sidecar 缓存按原字段名读写(与旧 TranscriptDigest 内部 state
    dict 同名,持久化文件字节兼容)。
    """
