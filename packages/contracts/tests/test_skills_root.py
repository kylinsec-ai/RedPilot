...
不存在的 `packages/worker/skills`，于是技能扫描静默退化成 0 个 —— 只打一条 warning，
...
class SkillsRootTests(unittest.TestCase):
    ...
    def test_walks_up_from_the_deepest_call_site(self):
        ...
        `adapter/solver/pi_agent.py` 距仓库根 6 层。此前上溯上限恰好是 6，
        即最深调用方用满最后一层、**零余量**：再加一层包目录（或再挪一次文件）
        就静默归零。上面那条守卫从较浅的 skill_loader 出发，测不出这一点。
        """
        start = _REPO / "packages/worker/ghost_worker/adapter/solver/pi_agent.py"
        self.assertEqual(skills_root(str(start)), str(_REPO / "skills"))
...
class SkillsAreActuallyLoadedTests(unittest.TestCase):
    ...
    def test_loader_and_pi_agree_on_what_counts_as_a_skill(self):
        ...
        判据本身已单源（两处都调 `ghost_contracts.paths.is_skill_dir`，其行为由
        下面那条用例守），但"两处都调了它"不等于"两处结果一致" —— 一边若改了
        过滤/遍历方式仍会错位。所以这里真的各跑一遍，比对集合。
        """
        from ghost_worker.adapter.skill_loader import SkillStore
        from ghost_worker.adapter.solver.pi_agent import _install_skills
    ...
    def test_is_skill_dir_is_the_single_predicate(self):
        ...
        from ghost_contracts.paths import is_skill_dir
        with tempfile.TemporaryDirectory() as tmp:
