...
from redpilot_contracts.vocabulary import (ACTIVE_PHASES, FLUSH_KINDS, PHASES,
                                            strip_for_snapshot, strip_out_of_band)

_CONTRACTS_DIR = Path(__file__).resolve().parent.parent / "redpilot_contracts"
...
def test_contracts_is_stdlib_only():
    ...
    for py in _CONTRACTS_DIR.rglob("*.py"):
        for mod in _iter_imports(py):
            root = mod.split(".")[0]
            assert root in _ALLOWED_STDLIB or root == "redpilot_contracts", \
...
def test_snapshot_keys_are_snake_case_unique():
    from redpilot_contracts.snapshot import LIVE_SNAPSHOT_KEYS
...
def test_safe_code_matches_local_scan_rule():
    from redpilot_contracts.paths import safe_code
