...
协作者(LiveState/LiveBus/RosterPoller/TranscriptDigest 全部构造注入、鸭子类型
使用 —— 本模块绝不 import tsecbench_worker,worker 的 driver 负责装配)。

与 obs/read.py 的协议契约(路由/SSE 帧/snapshot kind/资产表)单源
tsecbench_contracts —— :8080 与 :8090 的字节兼容由测试强制而非注释约定。
...
from tsecbench_contracts.assets import ASSET_RX, ASSET_TYPES
from tsecbench_contracts.paths import (DIGESTS_DIR, FLAG_FILES, FLAG_MAX_LINES,
                                       LIVE_DIR, TRANSCRIPT_FILENAME, safe_code)
from tsecbench_contracts.vocabulary import ACTIVE_PHASES, SNAPSHOT_KIND, \
