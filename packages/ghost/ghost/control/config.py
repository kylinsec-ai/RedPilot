...
class Settings:
    ...
    def from_env(cls, env_file: str | Path | None = None) -> Settings:
        ...
        database_path = str(Path(os.getenv("GHOST_DB_PATH", os.getenv("GHOST_DATABASE", cls.database_path))).expanduser())
        config_path = os.getenv("GHOST_CONFIG")
        inline_config = os.getenv("GHOST_TASKS_JSON")
        token_value = os.getenv("BENCHMARK_TOKEN", "").strip()
        token = token_value or None
        admin_value = os.getenv("GHOST_ADMIN_TOKEN", "").strip()
        admin_token = admin_value or None
        worker_value = os.getenv("GHOST_WORKER_TOKEN", "").strip()
        worker_token = worker_value or None
        public_value = os.getenv("GHOST_PUBLIC_BASE_URL", "").strip()
        ...
        try:
            max_active = int(os.getenv("GHOST_MAX_ACTIVE_CHALLENGES", "3"))
            port = int(os.getenv("PORT", "8000"))
        except ValueError as exc:
            raise ConfigurationError("numeric environment settings are invalid") from exc
        if max_active < 1:
            raise ConfigurationError("GHOST_MAX_ACTIVE_CHALLENGES must be at least 1")
