#!/usr/bin/env python3
"""obs/localserver — 本地实时源仪表板(stdlib 无依赖,worker 容器 :8080)。

这是 obs 读 API 的"本地实时数据源"变体:数据不来自 SQLite,而来自注入的
协作者(LiveState/LiveBus/RosterPoller 构造注入、鸭子类型使用 —— 本模块绝不
import ghost_worker,worker 的 driver 负责装配)。

依赖说明(架构边界):worker → obs.localserver 是唯一允许的 worker→obs 代码
import,且仅限驱动装配(serve_forever_in_thread)与 roster 快照函数的注入式
复用;worker 绝不 import obs.store/db/schema/ingest(无数据依赖)。本地仪表板
是 stdlib 诊断工具,不是数据链路:远端证据链只走 HTTP relay → obs ingest。

与 obs/read.py 的协议契约(路由/SSE 帧/snapshot kind/资产表)单源
ghost_contracts —— :8080 与 :8090 的字节兼容由测试强制而非注释约定。
...
from ghost_contracts.assets import ASSET_RX, ASSET_TYPES
from ghost_contracts.paths import (FLAG_FILES, FLAG_MAX_LINES,
                                       LIVE_DIR, TRANSCRIPT_FILENAME, safe_code)
from ghost_contracts.vocabulary import ACTIVE_PHASES, SNAPSHOT_KIND, \
    SSE_HEARTBEAT_S, strip_out_of_band

log = logging.getLogger("obs.localserver")
