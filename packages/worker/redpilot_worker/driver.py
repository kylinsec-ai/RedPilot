...
`redpilot_worker.orchestrator`，本模块不再有自己的 list 循环。

本地可观测性: redpilot.obs.localserver(注入式 stdlib 仪表板 :8080)
远端可观测性: redpilot_worker.relay(→ obs 平台 /api/internal/*)
...
from redpilot.obs.localserver import serve_forever_in_thread as _serve_local
from redpilot_contracts.paths import LIVE_DIR
...
log = logging.getLogger("redpilot_worker.driver")
