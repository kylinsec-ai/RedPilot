...
竞技场主循环（`redpilot_worker.orchestrator`）用**朋友自己的** `SolverBackend`
接口调引擎（`flag_format=` / `on_fact=` / `stop_check=` / `transcript_path=`），
而这里保留的是框架侧的两个东西：

- `SolveResult`：框架读的那个结果形状。**注意它与朋友
  `redpilot_worker.adapter.solver.SolveResult` 不是同一个类** —— 朋友版没有
...
- `solver/factory.create_solver()`：竞技场用它那边的工厂
  （`redpilot_worker.adapter.solver.factory.create_solver`），不再需要框架这一份。
"""
