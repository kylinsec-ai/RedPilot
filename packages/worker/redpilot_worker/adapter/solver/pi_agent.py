...
from redpilot_contracts.paths import HEARTBEAT_PATH  # 心跳路径单源(contracts)
...
def _install_skills(pi_home: str) -> int:
    ...
    try:
        ...
        # `redpilot_worker/adapter/solver/` 之后指向不存在的 `packages/worker/skills`
        # —— pi 的原生技能面静默归零（agent 拿不到任何 SKILL.md）。
        from redpilot_contracts.paths import skills_root as _skills_root
