...
sys.path，且不在 worker 镜像内），现搬进 `redpilot_worker` 成为包内模块：

- 删掉模块顶部的 `sys.path.insert(...)` —— 包内导入不需要它；
- `redpilot_worker.adapter.*` 的绝对导入保持原样，包内成立；
...
from redpilot_worker.adapter.config import SolverConfig, ControllerConfig, build_verifier_config
from redpilot_worker.adapter.progress import ChallengeProgress, extract_progress_from_result
from redpilot_worker.adapter.task import AgentTask
from redpilot_worker.adapter.verify import (Verifier, flag_confidence, flag_submission_key,
                            subagent_enabled,
                            ...
                            FlagEvidencePolicy)
from redpilot_worker.adapter.solver import create_solver, extract_flags, extract_handoff, SolveResult
from redpilot_worker.adapter.blackboard import Blackboard, goals_for_category
try:                                   # [B54] 监控缺失绝不能拖垮解题路径
    from redpilot_worker.adapter import hallucination as _hallu
except Exception:                      # pragma: no cover
    _hallu = None
from redpilot_worker.adapter.stoploss import StopLoss
from redpilot_worker.adapter.scheduler import run_fleet
from redpilot_worker.adapter.taskprompt import build_task_prompt, write_context_md, write_memory
from redpilot_worker.adapter.platform_client import (PlatformClient, RateLimitedClient, Challenge,
                                     SubmitResult, InvalidState, DuplicateSubmit,
                                     ChallengeNotFound, ResourceUnavailable, VpnCheckError)
from redpilot_worker.adapter import observability as obs
...
from redpilot_contracts.paths import HEARTBEAT_PATH
...
_STATUS: dict = {
    ...
    # phase 是**给观测面的**（redpilot_contracts.vocabulary.PHASES 的词表）。
...
# ── 框架观测桥（可选）────────────────────────────────────
# 装配层（redpilot_worker.driver）注入一个 StatusBridge，把上面这份状态转译成
...
def _monitor_loop(*, raw_client=None, watch_dir: str = "", stop_event=None):
    ...
    # 完整设计说明见 redpilot_worker/supervisor.py 顶部注释；改之前先读它。
...
def _flag_plaintext_rx(workdir: str):
    ...
    **只读展示**路径（`.heimdall.json` 的 why/evidence 是工具输出摘录，可能含
    ...
    """
    from redpilot_worker.adapter.compliance import flag_plaintext_rx
...
def _heimdall_init(llm) -> None:
    ...
    if llm is None:
        ...
        return
    try:
        from redpilot_worker.adapter.heimdall import Heimdall
        _HEIMDALL = Heimdall(
...
def _heimdall_map_for(workdir: str) -> str:
    ...
    if _HEIMDALL is None:
        return ""
    try:
        from redpilot_worker.adapter.heimdall import load_state, render
...
def _heimdall_observe(workdir: str, tpath: str, session_idx: int) -> None:
    ...
    if _HEIMDALL is None:
        return
    try:
        from redpilot_worker.adapter.heimdall import review
...
def _solve_one_unlocked(
    ...
    *,
    ...
    try:
        ...
        try:
            from redpilot_worker.adapter.solver.pi_agent import cleanup_instance_processes
...
def main():
    ...
    if verifier_cfg.is_usable():
        try:
            from redpilot_worker.adapter.llm import LLMClient
