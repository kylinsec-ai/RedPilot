...
def create_solver(model: str = "", skills_dir: str = "",
                  max_turns: int = 60, thinking: str = "") -> SolverBackend:
    """创建 Pi Agent 求解器后端"""
    log.info("solver backend: pi-agent (model=%s skills=%s thinking=%s)",
             model or "default", skills_dir or "none", thinking or "-")
