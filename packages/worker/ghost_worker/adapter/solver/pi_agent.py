...
def _install_subagents(pi_home: str) -> bool:
    ...
    try:
        ...
        _model_override = (os.environ.get("ANTHROPIC_MODEL", "")
                           or os.environ.get("ADAPTER_SOLVER_MODEL", "")
                           or "deepseek-v4-flash-0731").strip()
            ...
            try:
                ...
                try:
                    ...
                    while True:
                        ...
                        # 最多滞后 30s）。
                        ready, _, _ = select.select([proc.stdout], [], [], _stop_check_interval)
                        if not ready:
