...
（取值就是 `ghost_contracts.vocabulary.PHASES` 的词表）。本模块只做**改名**，
不做**推导** —— 这一条是刻意的：
...
from ghost_contracts.text import ERROR_HEAD_MAX, OUTPUT_TAIL_MAX, tail_text
from ghost_contracts.vocabulary import OUT_OF_BAND_PREFIX, PHASES
...
class StatusBridge:
    ...
    参数就是框架观测面既有的三个对象：
      ...
      - `relay`: `ghost_worker.relay.ObsRelay`（可选；用于把平台确认的明文 flag
        带外送进 Runs 历史）
    """
